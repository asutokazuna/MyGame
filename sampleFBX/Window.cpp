...
/* =================================== */
namespace {
	const TCHAR g_szProc[] = _T("PropCWindowProc");	// プロパティを識別する文字列
	const TCHAR g_szClass[] = _T("CWindow");
}

/* =================================== */
/* =====		静的メンバ		 ===== */
/* =================================== */
CWindow* CWindow::m_pMainWnd = nullptr;		// メイン ウィンドウ
...
bool CWindow::Attach(HWND hWnd)
{
	...
	if (::GetWindowLongPtr(m_hWnd, nIndex) != (LONG_PTR)WndMapProc)
		m_lpfnOldWndProc = (WNDPROC)::SetWindowLongPtr(m_hWnd, nIndex, (LONG_PTR)WndMapProc);
