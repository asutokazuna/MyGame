...
HRESULT LoadHullShader(LPCWSTR pwszPSFName, ID3D11HullShader** ppPixelShader)
{
	...
	if (IS_INTRESOURCE(pwszPSFName)) {
		...
		if (!pbData) {
			WCHAR wszMsg[_MAX_PATH * 2];
			swprintf_s(wszMsg, _countof(wszMsg), L"ハルシェーダ(ID=%d) 読込エラー", (int)(ULONG_PTR)pwszPSFName);
			MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
			return E_FAIL;
		}
	}
		...
		if (fp) {
			...
			if (FAILED(hr)) {
				WCHAR wszMsg[_MAX_PATH * 2];
				swprintf_s(wszMsg, _countof(wszMsg), L"ハルシェーダ(%s) コンパイル失敗", pwszPSFName);
				MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
				return hr;
			}
			pbData = (PBYTE)pCompiledShader->GetBufferPointer();
...
#else
			WCHAR wszMsg[_MAX_PATH * 2];
			swprintf_s(wszMsg, _countof(wszMsg), L"ハルシェーダ(%s) 読み込みエラー", pwszPSFName);
			MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
			return E_FAIL;
#endif
		}
	...
	if (FAILED(hr)) {
		WCHAR wszMsg[_MAX_PATH * 2];
		swprintf_s(wszMsg, _countof(wszMsg), L"ハルシェーダ(%s) 生成失敗", pwszPSFName);
		MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
		return hr;
	}
	return hr;
...
HRESULT LoadDomainShader(LPCWSTR pwszPSFName, ID3D11DomainShader** ppPixelShader)
{
	...
	if (IS_INTRESOURCE(pwszPSFName)) {
		...
		if (!pbData) {
			WCHAR wszMsg[_MAX_PATH * 2];
			swprintf_s(wszMsg, _countof(wszMsg), L"ドメインシェーダ(ID=%d) 読込エラー", (int)(ULONG_PTR)pwszPSFName);
			MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
			return E_FAIL;
		}
	}
...
#else
			WCHAR wszMsg[_MAX_PATH * 2];
			swprintf_s(wszMsg, _countof(wszMsg), L"ドメインシェーダ(%s) 読み込みエラー", pwszPSFName);
			MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
			return E_FAIL;
#endif
		}
	...
	if (FAILED(hr)) {
		WCHAR wszMsg[_MAX_PATH * 2];
		swprintf_s(wszMsg, _countof(wszMsg), L"ドメインシェーダ(%s) 生成失敗", pwszPSFName);
		MessageBoxW(hWnd, wszMsg, L"error", MB_OK);
		return hr;
	}
	return hr;
